...
idex_reg_t gen_control(Instruction instr) {
    ...
    if (op == 0x33) { // R-type
        idex.reg_write = true;
    } else if (op == 0x13 || op == 0x03 || op == 0x67) { // I-type, Load, JALR
        idex.reg_write = true;
        idex.alu_src = true;
        if (op == 0x03) {
            idex.mem_read = true;
...
exmem_reg_t stage_execute(idex_reg_t idex_reg, pipeline_wires_t* pwires_p) {
  ...
  exmem_reg.rs2_val = val_rs2; // Pass forwarded rs2 to Memory stage for Store instructions
...
void cycle_pipeline(regfile_t* regfile_p, Byte* memory_p, Cache* dcache_p, Cache* icache_p, pipeline_regs_t* pregs_p, pipeline_wires_t* pwires_p, bool* ecall_exit) {
  ...
  // MS2: Hazard detection is turned OFF for MS1
  #ifdef PRINT_STATS
  ...
  // MS2: Data forwarding is turned OFF for MS1
  #ifdef PRINT_STATS
  ...
  // MS2: Data Hazard Load-Use Stall Resolution is turned OFF for MS1
  #ifdef PRINT_STATS
  if (pwires_p->stall) {
    pwires_p->pc_src0 = pregs_p->ifid_preg.out.instr_addr; // Refetch same instruction
    pwires_p->pcsrc = false;
    
    pregs_p->ifid_preg.inp = pregs_p->ifid_preg.out; // Retain ID instruction
    
    memset(&pregs_p->idex_preg.inp, 0, sizeof(idex_reg_t)); // Insert NOP bubble
...
#else
  // MS1 & MS2: Standard Control Hazard Resolution
  if (pregs_p->memwb_preg.inp.branch_taken) {
    pwires_p->pcsrc = true;
    pwires_p->pc_src1 = pregs_p->memwb_preg.inp.branch_target;
    
    if (pregs_p->memwb_preg.inp.is_branch) branch_counter++;

    // MS2: Pipeline Flushing is turned OFF for MS1. 
    // In MS1, we must execute the delay slot instructions!
    #ifdef PRINT_STATS
    memset(&pregs_p->ifid_preg.inp, 0, sizeof(ifid_reg_t));
    pregs_p->ifid_preg.inp.instr.bits = 0x00000013;
    memset(&pregs_p->idex_preg.inp, 0, sizeof(idex_reg_t));
